...
#define HTML_400 "wwwroot/400.html"
#define HTML_404 "wwwroot/404.html"
#define HTML_500 "wwwroot/500.html"
#define HTML_503 "wwwroot/503.html"
const char *ErrLevel[] = {
	"Normal",
	"Warning",
	"Error",
...
public:
	...
	static std::string SuffixToContent(std::string& suffix)
	{
		...
		if (suffix == ".jpg") { return "application/x-jpg"; }
		return "text/html";
	}
	...
	static int FileSize(std::string &ExceptPath)
	{
		struct stat st;
		stat(ExceptPath.c_str(), &st);
		return st.st_size;
	}
};
class SocketApi
...
public:
	...
	static void HandlerRequest(int sock)
	{
		...
		if (code != 200)
		{
			std::string ExceptPath = Util::CodeToExceptFile(code);
			int rs = Util::FileSize(ExceptPath);
			rsp->SetPath(ExceptPath);
			rsp->SetResourceSize(rs);
